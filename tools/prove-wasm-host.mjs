...
function generatedBytes(initialFile, scenarioId) {
  assert.ok(
    Number.isInteger(initialFile.bytes) && initialFile.bytes > 0,
    `${scenarioId} ${initialFile.name} generated byte length`,
  );
  assert.ok(
    Number.isInteger(initialFile.fillByte) &&
      initialFile.fillByte >= 0 &&
      initialFile.fillByte <= 0xff,
    `${scenarioId} ${initialFile.name} fillByte`,
  );
  const bytes = new Uint8Array(initialFile.bytes).fill(initialFile.fillByte);
  for (const patch of initialFile.patches ?? []) {
    assert.ok(
      Number.isInteger(patch.offset) && patch.offset >= 0,
      `${scenarioId} ${initialFile.name} patch offset`,
    );
    assert.ok(
      Array.isArray(patch.bytes) &&
        patch.bytes.every(
          (byte) => Number.isInteger(byte) && byte >= 0 && byte <= 0xff,
        ),
      `${scenarioId} ${initialFile.name} patch bytes`,
    );
    assert.ok(
      patch.offset + patch.bytes.length <= bytes.length,
      `${scenarioId} ${initialFile.name} patch bounds`,
    );
...
async function proveCpm() {
  ...
  for (const scenarioPath of scenarioPaths) {
    ...
    for (const initialFile of scenario.initialFiles ?? []) {
      ...
      } else if (initialFile.encoding === "generated-bytes") {
        assert.equal(
          initialFile.path,
          undefined,
          `${scenario.id} generated files have no source path`,
        );
        bytes = generatedBytes(initialFile, scenario.id);
      } else {
        assert.fail(`${scenario.id} unsupported initial file encoding`);
      }
